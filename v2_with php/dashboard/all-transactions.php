...
<!-- Mirrored from andit.co/projects/html/andshop/andshop-dashboard/all-transactions.html by HTTrack Website Copier/3.x [XR&CO'2014], Mon, 27 Feb 2023 04:18:39 GMT -->
 ...
 while($row=mysqli_fetch_array($result))
{
	...
	$query4="SELECT * FROM `tbl_invoices` where invoice_oid='".$row['order_id']."'";
	$result4 = mysqli_query($con, $query4);
	$row4=mysqli_fetch_array($result4);
	$i=$i+1;
echo '<tr>';
...
echo"<td>". $row4['invoice_date']."</td>";
echo  "<td>Rs.".$row['order_quantity']*$row2['prod_price'];
echo"</td>";

echo "<td>";
echo  $row4['invoice_payment_method'];
echo"</td>";
echo '<td><div class="form-check form-switch"><input onclick="toggle('.$row4['invoice_id'].')" type="checkbox" id="status" class="form-check-input"';
	if($row4['invoice_status']==1){
		echo ' checked ';
	}
	else{
		echo ' ';	
